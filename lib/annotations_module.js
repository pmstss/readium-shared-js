...
var EpubAnnotationsModule = function (contentDocumentFrame, bbPageSetView, annotationCSSUrl) {
    ...
    EpubAnnotations.Helpers = {
        ...
        armImages: function (contentDocumentFrame, bbPageSetView) {
            $('img', contentDocumentFrame.contentDocument).on("dblclick", function(event) {
                ...
                // Know requirements
                bbPageSetView.trigger("imgDblClicked", event.type, event.target.src, event.target.alt, 
    ...
    EpubAnnotations.HighlightGroup = Backbone.Model.extend({
        ...
        highlightGroupCallback : function (event) {
            ...
            if (event.type === "click") {
                that.get("bbPageSetView").trigger("annotationClicked", that.get("type"),
                                                  that.get("CFI"), that.get("id"), event, documentFrame);
                return;
            }

            if (event.type === "touchend" || event.type === "touchmove") {
                that.get("bbPageSetView").trigger("annotationTouched", that.get("type"),
                    that.get("CFI"), that.get("id"), event, documentFrame);
                return;
            }
            
            if (event.type === "contextmenu") {
                that.get("bbPageSetView").trigger("annotationRightClicked", that.get("type"),
                                                  that.get("CFI"), that.get("id"), event , documentFrame);
                return;
            }
            ...
            if (event.type === "mouseenter") {
                that.get("bbPageSetView").trigger("annotationHoverIn", that.get("type"),
                                                  that.get("CFI"), that.get("id"), event, documentFrame);
            } else if (event.type === "mouseleave") {
                that.get("bbPageSetView").trigger("annotationHoverOut", that.get("type"),
                                                  that.get("CFI"), that.get("id"), event, documentFrame);
            }
        ...
        constructHighlightViews : function () {
            ...
            _.each(selectedNodes, function (node) {
                ...
                // selected text will be kept in HighlightGroup.highlightText
                that.selectionText = range.toString();
    ...
    EpubAnnotations.ReflowableAnnotations = Backbone.Model.extend({
        ...
        initialize : function (attributes, options) {
            ...
            epubWindow.addEventListener("mouseup", function(event) {
                ...
                if (range.startOffset - range.endOffset) {
                    self.get("bbPageSetView").trigger(
        ...
        addHighlightsForText: function(text, spineIdRef, type, styles) {
            // array of "partial CFI" to the created highlights
            var cfis = [];
 
            // get all the text nodes in contentDocumentFrame (jQuery object)
            var contentDocumentFrame = this.get("contentDocumentFrame");
            var textNodes =  $(contentDocumentFrame.contentDocument).find(
                ":not(iframe)").addBack().contents().filter(
                    function() {
                        return this.nodeType === 3;
                    });

            // we need a "unique" id for every highlight, so we are
            // assuming that there could be only one text highlighting for the "type"
            var idPrefix = spineIdRef + "_" + type + "_";
            var counter = 1;
            var hlId;
            var that = this;
            ...
            _.each(textNodes, function (node) {
                ...
                var indices = [];
                for(var pos = nodeText.indexOf(textLower); pos !== -1; pos = nodeText.indexOf(textLower, pos + 1)) {
                    indices.push(pos);
                }
                ...
                _.each(indices, function(index) {
                    ...
                    var cfi = that.epubCFI.generateCharOffsetRangeComponent(
                        $node[0], index, $node[0], index + text.length,
    ...
    EpubAnnotations.Helpers.armImages(contentDocumentFrame, bbPageSetView);
    ...
    return {
        ...
        addHighlightsForText: function (text, spineIdRef, type, styles) {
            return reflowableAnnotations.addHighlightsForText(text, spineIdRef, type, styles);
        },
    };
};
