...
var EpubAnnotationsModule = function (contentDocumentFrame, annotationsManagerView, annotationCSSUrl) {
    var EpubAnnotations = {};
    ...
    EpubAnnotations.ReflowableAnnotations = Backbone.Model.extend({
        ...
        getTextNodesOfCurrentPage: function() {
            ...
            if (!firstVisibleCfi.contentCFI) {
                console.warn("addHighlightsForText: firstVisibleCfi - invalid contentCFI!");
                return undefined;
            }

            var lastVisibleCfi = ReadiumSDK.reader.getLastVisibleCfi();
            if (!lastVisibleCfi.contentCFI) {
                console.warn("addHighlightsForText: lastVisibleCfi - invalid contentCFI!");
                return undefined;
            }

            // get DOM range for the page
            var range = ReadiumSDK.reader.getDomRangeFromRangeCfi(firstVisibleCfi, lastVisibleCfi, false);

            // wrap DOM range into "rangy" range and filter text nodes
            var textNodes = (new rangy.WrappedRange(range)).getNodes().filter(
                function(node) {
        ...
        addHighlightsForText: function(text, spineIdRef, type, styles) {
            ...
            idPrefix = 'xxxxxxxx'.replace(/[x]/g, function(c) {
