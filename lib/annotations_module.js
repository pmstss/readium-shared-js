...
var EpubAnnotationsModule = function (contentDocumentFrame, annotationsManagerView, annotationCSSUrl, spineItem) {
    ...
    EpubAnnotations.HighlightGroup = Backbone.Model.extend({
        ...
        elementNodeAllowedTags: ["img", "video", "audio"], //in lowercase

        boundHighlightContainerEvents: ["click", "touchend", "touchmove", "contextmenu", "mouseenter", "mouseleave", "mousemove"],
        getBoundHighlightContainerEvents: function () {
            var namespace = ".rdjsam";
            return this.boundHighlightContainerEvents.join(namespace + " ") + namespace;
        },
        ...
        constructHighlightViews : function () {
            ...
            if (rangeInfo && rangeInfo.startNode === rangeInfo.endNode) {
                ...
                if (node.nodeType === 3) {
                    ...
                    _.each(rects, function (rect) {
                        rectTextList.push({
                            rect: that.normalizeRectangle(rect),
                            ancestorEl: range.commonAncestorContainer.nodeType === Node.ELEMENT_NODE
            ...
            inferredLines = inferrer.inferLines(_.pluck(rectTextList,"rect"));
            _.each(inferredLines, function (line, index) {
                ...
                var highlightView = new EpubAnnotations.HighlightView({
                    ...
                    styles: _.extend({"z-index": "1000", "pointer-events": "none"}, that.get('styles')),
                    highlightGroupCallback : that.highlightGroupCallback,
                    callbackContext : that
                });

                var highlightView2 = new EpubAnnotations.HighlightView({
                    highlightId: that.get('id'),
                    CFI : that.get("CFI"),
                    type: that.get("type"),
                    top : highlightTop,
                    ...
                    height : highlightHeight,
                    width : highlightWidth,
                    styles : _.extend({"z-index":"-1", "pointer-events":"none"},that.get('styles'))
                });

                that.get("highlightViews").push(highlightView);
                that.get("highlightViewsSecondary").push(highlightView2);
            });
            ...
            function forwardEvents(rect, e) {

                if (pointRectangleIntersection({x: e.pageX, y: e.pageY}, rect)) {
                    if (e.type === "click" && !e.target.ownerDocument.getSelection().getRangeAt(0).collapsed) {
                ...
                } else if ((e.type === "mousemove" || e.type === "mouseleave") && mouseEntered) {
                    e.type = "mouseleave";
                    mouseEntered = false;
                    that.highlightGroupCallback(e);
                }
        ...
        destroyCurrentHighlights : function () {
            var that = this;
            _.each(this.get("highlightViews").concat(this.get("highlightViewsSecondary")), function (highlightView) {
            ...
            this.get("highlightViewsSecondary").length = 0;
        },

        renderHighlights : function (viewportElement) {
            _.each(this.get("highlightViews").concat(this.get("highlightViewsSecondary")), function (view, index) {
