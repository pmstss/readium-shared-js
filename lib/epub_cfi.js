...
EPUBcfi.Interpreter = {
    ...
    injectRangeElements : function (rangeCFI, contentDocument, startElementToInject, endElementToInject, classBlacklist, elementBlacklist, idBlacklist) {
        ...
        getRangeTextNodes: function (rangeCFI, contentDocument, classBlacklist, elementBlacklist, idBlacklist) {
            ...
            return {
                ...
                startOffset: CFIAST.cfiString.range1.termStep.offsetValue,
                endNodes: _.toArray($range2Nodes),
                endOffset: CFIAST.cfiString.range2.termStep.offsetValue
            };
        },
...
EPUBcfi.CFIAssertionError = function (expectedAssertion, targetElementAssertion, message) {
    ...
    // ------------------------------------------------------------------------------------ //
        ...
        generateMixedRangeComponent: function (rangeStartElement, startOffset, rangeEndElement, endOffset, commonAncestor, classBlacklist, elementBlacklist, idBlacklist) {
            ...
            function validateAndGetNodeType(node) {
                if (self.validateTextNode(node)) {
                    return 3;
                } else if (self.validateElementNode(node)) {
                    return 1;
                } else {
            ...
            range1ElementType = validateAndGetNodeType(rangeStartElement);
            range2ElementType = validateAndGetNodeType(rangeEndElement);

            var sameParent = $(rangeStartElement).parent()[0] === $(rangeEndElement).parent()[0];
            ...
            function getRangeCfiPart(node, nodeType, nodeOffset) {
                var cfiPart;
                if (nodeType === 3 && !sameParent) {
                    // Generate terminating offset and range 1
                    offsetStep = self.createCFITextNodeStep($(node), nodeOffset, classBlacklist, elementBlacklist, idBlacklist);
                    cfiPart = self.createCFIElementSteps($(node).parent(), commonAncestor, classBlacklist, elementBlacklist, idBlacklist) + offsetStep;
                } else if (nodeType === 3 && sameParent) {
                    cfiPart = self.createCFITextNodeStep($(node), nodeOffset, classBlacklist, elementBlacklist, idBlacklist);
                } else if (nodeType === 1 && nodeOffset === 0) {
                    cfiPart = self.createCFIElementSteps($(node), commonAncestor, classBlacklist, elementBlacklist, idBlacklist);
                } else {
                    ...
                    cfiPart = self.createCFIElementSteps($(childNode), commonAncestor, classBlacklist, elementBlacklist, idBlacklist);
                }
                return cfiPart;
            }

            var range1Cfi = getRangeCfiPart(rangeStartElement, range1ElementType, startOffset);
            var range2Cfi = getRangeCfiPart(rangeEndElement, range2ElementType, endOffset);
