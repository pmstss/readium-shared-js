...
EPUBcfi.Interpreter = {
    ...
    injectRangeElements : function (rangeCFI, contentDocument, startElementToInject, endElementToInject, classBlacklist, elementBlacklist, idBlacklist) {
        ...
        getRangeTargetNodes: function (rangeCFI, contentDocument, classBlacklist, elementBlacklist, idBlacklist) {
            ...
            // Interpret the first range local_path
            $range1Nodes = this.interpretLocalPath(CFIAST.cfiString.range1, 0, $currElement, classBlacklist, elementBlacklist, idBlacklist);

            // Interpret the second range local_path
            $range2Nodes = this.interpretLocalPath(CFIAST.cfiString.range2, 0, $currElement, classBlacklist, elementBlacklist, idBlacklist);

            startOffset = parseInt(CFIAST.cfiString.range1.termStep.offsetValue);
            endOffset = parseInt(CFIAST.cfiString.range2.termStep.offsetValue);

            if(!CFIAST.cfiString.range1.termStep.offsetValue && !CFIAST.cfiString.range2.termStep.offsetValue){
                //this is to check for cases where we get a cfi with 2 elements in the range
                // like this: epubcfi(/99!/4/2,/12,/16)
                if ($range1Nodes
                    && $range2Nodes
                    && $range1Nodes.length === 1
                    && $range2Nodes.length === 1
                    && $range1Nodes[0].nodeType === 1
                    && $range2Nodes[0].nodeType === 1) {
                    startOffset = 0;
                    endOffset = 1;
                } else {
                    throw Error('CFI getRangeTargetNodes: invalid rangeCFI');
                }
            }
            ...
            // Return the element that was injected into
            return {
                startNodes: _.toArray($range1Nodes),
                startOffset: startOffset ? startOffset : 0,
                endNodes: _.toArray($range2Nodes),
...
EPUBcfi.CFIAssertionError = function (expectedAssertion, targetElementAssertion, message) {
    ...
    // ------------------------------------------------------------------------------------ //
        ...
        //REFACTORING CANDIDATE: This method is too complex!
...
//            else if ($(rangeStartElement).parent()[0] === rangeEndElement){
//                sameParent = true;
//            }else if ($(rangeEndElement).parent()[0] === rangeStartElement){
//                sameParent = true;
//            }
            ...
            var range1Cfi = this.generateRangeCfiComponentFromNode(rangeStartElement, range1ElementType, startOffset, sameParent, commonAncestor, classBlacklist, elementBlacklist, idBlacklist);
            var range2Cfi = this.generateRangeCfiComponentFromNode(rangeEndElement, range2ElementType, endOffset, sameParent, commonAncestor, classBlacklist, elementBlacklist, idBlacklist);
    ...
    generateCharOffsetRangeComponent : function (rangeStartElement, startOffset, rangeEndElement, endOffset, classBlacklist, elementBlacklist, idBlacklist) {
        ...
        var commonCFIComponent;
            ...
            if (!this.validateTextNode(rangeStartElement)) {
                throw new EPUBcfi.NodeTypeError(rangeStartElement, "Cannot generate a character offset from a starting point that is not a valid text node");
            } else if (!this.validateTextNode(rangeEndElement)) {
                throw new EPUBcfi.NodeTypeError(rangeEndElement, "Cannot generate a character offset from an ending point that is not a valid text node");
            }
        ...
        if ($(rangeStartElement).parent()[0] === $(rangeEndElement).parent()[0]) {
            ...
            // Create a document range to find the common ancestor
            docRange = rangeStartElement.ownerDocument.createRange();
            docRange.setStart(rangeStartElement, startOffset);
            docRange.setEnd(rangeEndElement, endOffset);
            commonAncestor = docRange.commonAncestorContainer;
    ...
    generateElementRangeComponent : function (rangeStartElement, rangeEndElement, classBlacklist, elementBlacklist, idBlacklist) {
        ...
        var commonCFIComponent;
            ...
            if (!this.validateElementNode(rangeStartElement)) {
                throw new EPUBcfi.NodeTypeError(rangeStartElement, "CFI target element is not valid");
            } else if (!this.validateElementNode(rangeEndElement)) {
                throw new EPUBcfi.NodeTypeError(rangeEndElement, "CFI target element is not valid");
            }
    ...
    generateCharacterOffsetCFIComponent : function (startTextNode, characterOffset, classBlacklist, elementBlacklist, idBlacklist) {
        ...
        var packageDocCFI;
            ...
            this.validateTextNode(startTextNode, characterOffset);
    ...
    generateElementCFIComponent : function (startElement, classBlacklist, elementBlacklist, idBlacklist) {
        ...
        var packageDocCFI;
            ...
            this.validateElementNode(startElement);
    ...
    // ------------------------------------------------------------------------------------ //
        ...
        generateRangeCfiComponentFromNode: function (node, nodeType, nodeOffset, sameParent, commonAncestor, classBlacklist, elementBlacklist, idBlacklist) {

            var cfiPart;
            if (nodeType === 3 && !sameParent) {
                // Generate terminating offset and range 1
                ...
                if (childNode.nodeType === 3){
                    cfiPart = this.createCFITextNodeStep($(childNode), childNode.length, classBlacklist, elementBlacklist, idBlacklist);
    ...
    global.EPUBcfi = EPUBcfi =  {
        ...
        generateMixedRangeComponent: function (rangeStartElement, startOffset, rangeEndElement, endOffset, commonAncestor, classBlacklist, elementBlacklist, idBlacklist) {
            return generator.generateMixedRangeComponent(rangeStartElement, startOffset, rangeEndElement, endOffset, commonAncestor, classBlacklist, elementBlacklist, idBlacklist);
        },
        getRangeTargetNodes: function (rangeCFI, contentDocument, classBlacklist, elementBlacklist, idBlacklist) {
            return interpreter.getRangeTargetNodes(rangeCFI, contentDocument, classBlacklist, elementBlacklist, idBlacklist);
        }
    };
